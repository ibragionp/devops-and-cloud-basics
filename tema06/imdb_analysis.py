...
name_basics_file_path = '/datasets/name.basics/name.basics.tsv'
title_basics_file_path = '/datasets/title.basics/title.basics.tsv'
title_principals_file_path = '/datasets/title.principals/title.principals.tsv'
...
def import_datasets(dataset_path, cols):
    df = read_csv(files_path + dataset_path, 
                  sep = '\t', 
                  header = 0, 
                  usecols = cols,
                  low_memory = False)
...
def filter_df_title(filter_lst):
    ...
    df = import_datasets(title_basics_file_path, cols_title_basics)
...
def filter_df_title_principals(filter_lst):
    ...
    df = import_datasets(title_principals_file_path, cols_title_principals)
...
def filter_df_name(filter_lst):
    ...
    df = import_datasets(name_basics_file_path, cols_name_basics)
